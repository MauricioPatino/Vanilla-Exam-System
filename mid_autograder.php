...
if ($data["requestType"]=="autograder") {
  ...
  foreach ($answers as $answer) {
	...
	$partialPoints = $points / (count($testCaseResults)); // i.e. we ll give equal number of points for each test case 
		...
		if ($exitcode==0) { // exit code 0 means there was no compilation error. Proceed with comparing outputs for each test case
		  ...
		  if ($numPassed == $totalCases) {
			...
			$comment = "$numPassed out of $totalCases tests passed." .
				...
				"\nExpected:\n" . implode("\n", $testCaseResults);
			if ($numPassed > 0) {
				$earnedPoints += ($partialPoints * $numPassed);
			} else {
				  $pos = strpos($functionBody, $functionName);
				  if ($pos !== false) {
					$comment = $comment . "\nFunction name is correct, giving partial credit.";
					$earnedPoints += ($points / 4);
				  }
			}
		  ...
		  $pos = strpos($functionBody, $functionName);
		  if ($pos !== false) {
			$comment = $comment . " Function name is correct, giving partial credit.";
			$earnedPoints += ($points / 4);
		  }
		}
	} else {
